...
import org.example.wecambackend.dto.response.organization.SubOrganizationRequestDetailResponse;
import org.example.wecambackend.dto.response.organization.SubOrganizationRequestListResponse;
import org.example.wecambackend.dto.response.organization.SubOrganizationResponse;
import org.example.wecambackend.dto.response.organization.SubOrganizationDetailResponse;
...
public class AdminOrganizationController {
    ...
    @GetMapping("/requests")
    @Operation(summary = "하위 학생회 워크스페이스 생성 요청 목록 조회",
            description = "현재 접속한 학생회의 하위 조직에서 요청한 워크스페이스 생성 요청 목록을 조회합니다.",
            parameters = {
                    @Parameter(name = "X-Council-Id", description = "현재 접속한 학생회 ID", in = ParameterIn.HEADER),
                    @Parameter(name = "councilName", description = "학생회 이름", in = ParameterIn.PATH)
            })
    public BaseResponse<List<SubOrganizationRequestListResponse>> getSubOrganizationRequestList(
            @PathVariable String councilName,
            @AuthenticationPrincipal UserDetails userDetails) {

        log.info("하위 학생회 워크스페이스 생성 요청 목록 조회: councilName={}, user={}",
                councilName, userDetails.getUsername());

        List<SubOrganizationRequestListResponse> responses = organizationRequestAdminService.getSubOrganizationRequestList();
    ...
    public BaseResponse<SubOrganizationRequestDetailResponse> getOrganizationRequestDetail(
            @PathVariable String councilName,
        ...
        SubOrganizationRequestDetailResponse response = organizationRequestAdminService.getOrganizationRequestDetail(requestId, councilName);
        return new BaseResponse<>(response);
    }

    @GetMapping("/request/{requestId}/file/{fileId}/download")
